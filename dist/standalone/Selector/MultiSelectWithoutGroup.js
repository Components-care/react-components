...
var MultiSelectWithoutGroup = function (props) {
    var onSelect = props.onSelect, selected = props.selected, disabled = props.disabled, enableIcons = props.enableIcons, loadDataOptions = props.loadDataOptions, getIdOfData = props.getIdOfData, refreshToken = props.refreshToken, switchValue = props.switchValue, 
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    classProps = props.classes, otherProps = __rest(props, ["onSelect", "selected", "disabled", "enableIcons", "loadDataOptions", "getIdOfData", "refreshToken", "switchValue", "classes"]);
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    ...
                    setDataOptions(__spreadArray(__spreadArray([], dataOptions), [entryToDelete]));
    ...
    var multiSelectHandler = useCallback(function (data) {
        if (!data)
            return;
        var selectedOptions = __spreadArray(__spreadArray([], selected), [data]);
        if (onSelect)
            onSelect(selectedOptions);
    }, [onSelect, selected]);
    var onLoad = useCallback(function (query) { return loadDataOptions(query, !!switchValue); }, [loadDataOptions, switchValue]);
    return (React.createElement(Typography, { component: "div" },
        React.createElement(BaseSelector, __assign({}, otherProps, { onLoad: onLoad, selected: null, onSelect: multiSelectHandler, refreshToken: (refreshToken !== null && refreshToken !== void 0 ? refreshToken : "") +
                ...
                (switchValue !== null && switchValue !== void 0 ? switchValue : false).toString(), variant: "standard", startAdornment: React.createElement(SearchIcon, { color: "primary" }), freeSolo: true, displaySwitch: false, getIdOfData: getIdOfData, filterIds: selected.map(getId) })),
        React.createElement(InlineSwitch, { visible: !!props.displaySwitch, value: !!switchValue, onChange: props.setSwitchValue, label: props.switchLabel, classes: classes },
            React.createElement(React.Fragment, null, selected.map(function (data, index) {
